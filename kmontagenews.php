...
if ($proxy_action !== '') {
    ...
    if ($proxy_action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $payload['mode'] = 'news_opinions';
        if (empty($payload['video_style'])) { $payload['video_style'] = 'ai_avatar_news_explainer'; }
        ...
        $res = kmontage_api('GET', '/api/jobs?mode=news_opinions&limit=' . $limit, null, 20);
...
<header>
  <a class="brand" href="<?php echo h($THIS_FILE); ?>" aria-label="Kurage Montage トップへ">
    ...
    <div><strong>Kurage Montage News</strong><span>ニュースURLから反応紹介ショート動画生成</span></div>
  </a>
  <div class="userbar">
    <span class="badge <?php echo $api_ok ? 'api-ok' : 'api-err'; ?>"><?php echo $api_ok ? 'API ●' : 'API ×'; ?></span>
    <?php if ($logged_in): ?>
      <span class="name">@<?php echo h($session_user); ?></span><a class="btn-sm" href="?kmontagenews_logout=1">logout</a>
    <?php else: ?>
      <a class="btn-sm" href="?kmontagenews_login=1">Xでログイン</a>
    <?php endif; ?>
  </div>
</header>
...
<div class="wrap">
  <section class="hero">
    <div class="hero-card">
      <div class="eyebrow">Kurage Montage News</div>
      <h1>ニュースURLから、Kurageが反応まで紹介するショート動画を生成。</h1>
      <p class="lead">ニュース記事やX記事、ブログ、PDFのURLを貼るだけで、ニュース本文を取得し、Kurage AgentReachでX・YouTube・ブログなどの反応を集め、Kurageアバターがニュースとみんなの意見を紹介する2分程度のショート動画を生成します。</p>
    </div>
  </section>
  ...
  <section class="panel login">
    <h2>ログインが必要です</h2>
    <p>ニュース反応ショート動画を生成するにはログインしてください。</p>
    <a class="btn btn-primary" href="?kmontagenews_login=1">Xでログインして始める</a>
  </section>
  <?php else: ?>

  <section class="panel">
    <div class="panel-head"><span>ニュース反応ショート生成</span><small>ニュースURL / X記事 / ブログ / PDF</small></div>
    <div class="panel-body">
      <div class="input-row">
        <input id="source-url" type="url" placeholder="https://example.com/news または https://x.com/...">
        <button id="generate" class="btn-primary">生成する</button>
      </div>
      <div class="hint">ニュース取得、Kurage AgentReachによる反応収集、Gemma 12Bでの台本生成、Kurage動画生成を順番に実行します。Xの取得に失敗した場合も、Web・YouTubeの反応が十分なら生成できます。</div>
  ...
  <section class="panel">
    ...
    <div class="panel-body">
      ...
      <div id="summary" class="summary">ニュース要約、収集した意見、生成された台本がここに表示されます。</div>
...
function statusLabel(job){ const labels = {queued:'待機中',analyzing:'ニュース取得中',downloading:'元動画取得中',transcribing:'文字起こし中',researching:'意見収集中',planning:'台本生成中',generating:'Kurage動画生成中',done:'完了',error:'エラー'}; return labels[job.status] || job.status || '不明'; }
...
function renderJob(job){
  ...
  $('summary').textContent = job.summary || job.reference_analysis?.core_claim || job.opinion_research?.summary || job.analysis?.reference_analysis?.core_claim || job.transcript_preview || '解析中です。';
...
$('generate').addEventListener('click', async () => {
  ...
  try { const sameLoadedUrl = currentJobId && currentJobUrl && url === currentJobUrl; const endpoint = sameLoadedUrl ? `<?php echo h($THIS_FILE); ?>?proxy=regenerate&job_id=${encodeURIComponent(currentJobId)}` : '<?php echo h($THIS_FILE); ?>?proxy=create'; const data = await fetchJson(endpoint, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({url, mode:'news_opinions', vtuber_mode:true, video_style:'ai_avatar_news_explainer'})}); currentJobId = data.job_id; currentJobUrl = url; message(`${sameLoadedUrl ? '上書き再生成' : 'ジョブ開始'}: ${currentJobId}`); clearInterval(pollTimer); await poll(currentJobId); pollTimer = setInterval(() => poll(currentJobId), 5000); }
...
$('delete').addEventListener('click', async () => { if (!currentJobId || !confirm('この生成ジョブとKurage動画を削除しますか？')) return; await fetchJson(`<?php echo h($THIS_FILE); ?>?proxy=delete&job_id=${encodeURIComponent(currentJobId)}`, {method:'POST'}); currentJobId = null; currentJobUrl = ''; $('job-id').textContent='削除済み'; $('status').textContent='deleted'; $('title').textContent='タイトルはここに表示されます'; $('summary').textContent='ニュース要約、収集した意見、生成された台本がここに表示されます。'; $('script').innerHTML=''; $('player').style.display='none'; setActions(false); await loadHistory(); });
...
async function loadHistory(){
  const data = await fetchJson('<?php echo h($THIS_FILE); ?>?proxy=jobs&mode=news_opinions&limit=20');
  const box = $('history');
  box.innerHTML = '';
